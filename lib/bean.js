...
function Board(options){
  ...
  function reconnect(bean){
      console.log('>> [OK] Reconnecting', options.uuid);
      self.Bean.discoverById(options.uuid, discoveryService);
   };
   ...
   function discoveryService(bean){
     if(options.uuid === undefined || options.uuid == bean._peripheral.id){
        console.log(">> [OK] Discover ", bean._peripheral.id);
        ...
        bean.on("disconnect", function(){
            console.log(">> [OK] Bean disconnected", bean._peripheral.id);
            //console.log(options.uuid);
            reconnect(bean);
            //self.Bean.discoverById(options.uuid, discoveryService);
            //self.Bean.discoverAll(discoveryService);
          //process.exit();
        });
    }
